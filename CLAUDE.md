...
- **Deployment:** Vercel (direct Git integration, no GitHub Actions needed). Cron jobs defined in `vercel.json`.
...
- There should be NO `.github/workflows/ci.yaml` — Vercel deploys directly from the repo. If this file exists, delete it.
...
- When creating branches, ALWAYS branch from `origin/master` (not local master) to avoid GitHub "nothing to compare" issues.
